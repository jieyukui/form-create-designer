...
export class ContextAnalyzer {
    ...
    _isForLoopDeclaration(lineText, col, pos, doc) {
        ...
        for (let i = match[0].length; i < afterFor.length; i++) {
            const ch = afterFor[i];
            if (ch === '(') parenDepth++;
            else if (ch === ')') { parenDepth--; if (parenDepth === 0) { declEnd = forIndex + i; break; } }
            else if (ch === ';' && parenDepth === 1) { declEnd = forIndex + i; break; }
        }
...
| 方法 | 检测场景 | 示例 |
|------|----------|------|
| `_isSimpleDeclaration` | `const`/`let`/`var` 声明 | `var x =`, `const { y } =` |
| `_isForLoopDeclaration` | `for` 循环声明 | `for (var i = 0;`) |
| `_isForInOfDeclaration` | `for...in`/`for...of` | `for (var k in obj)` |
| `_isCatchParameter` | `catch` 参数 | `catch (err)` |
| `_isMultilineDeclaration` | 多行声明续行 | `var a,\n    b` |
| `_isInAssignmentLeft` | 赋值左侧 | `x = 1`（非声明） |
| `_isInImportExport` | `import`/`export` | `import { x }` |
| `_isInFunctionParams` | 函数参数 | `(a, b)` |
| `_isInComment` | 注释 | `// comment`, `/* */` |
| `_isInString` | 字符串 | `'str'`, `"str"` |
| `_isInTemplateExpression` | 模板表达式内 | `` `...${expr}` `` |
| `_detectPropertyAccess` | 属性访问 | `obj.x`, `arr[0]` |
| `_isInPropertyName` | 对象属性名 | `{ key: val }` |

现在 `var` 以及所有相关的声明场景都已完整覆盖。